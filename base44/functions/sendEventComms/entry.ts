...
// Bounded to 50 recipients per call. Reaches registered app users reliably.
  ...
  try {
    ...
    const html = `<div style="font-family:Inter,Arial,sans-serif;max-width:560px;margin:auto">
      <div style="background:linear-gradient(135deg,#0b2a47,#12608f);color:#fff;padding:24px;border-radius:14px 14px 0 0">
        <h1 style="margin:0;font-size:20px">${event.name}</h1>
      ...
      <div style="border:1px solid #e2e8f0;border-top:none;border-radius:0 0 14px 14px;padding:24px">
        <h2 style="margin:0 0 12px;font-size:16px;color:#0b2a47">${subject}</h2>
        <div style="font-size:14px;line-height:1.6;color:#334155;white-space:pre-line">${message}</div>
        <p style="margin-top:24px;font-size:12px;color:#94a3b8">Sent by ${user.full_name || 'Big Bay Events'} via the Big Bay Events OS.</p>
      </div>
    </div>`;
