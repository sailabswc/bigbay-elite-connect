...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await appRuntime.auth.register({ email, password });
      if (result?.access_token) appRuntime.auth.setToken(result.access_token);
  ...
  const handleGoogle = () => appRuntime.auth.loginWithProvider("google", returnTo);
  ...
  return (
    <AuthLayout
      ...
      footer={
        <>
          ...
          <Link to={"/login" + (returnTo !== "/" ? "?returnTo=" + encodeURIComponent(returnTo) : "")} className="text-primary font-medium hover:underline">
            Log in
          </Link>
        </>
      }
    >
      ...
      <div className="mb-4 rounded-lg bg-muted/50 p-3 text-sm text-muted-foreground">
        Local mode is active. Your account is created on this device without email verification.
      </div>
